...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _openDeviceDetail(Product product) {
    AppUtils.vibrate();
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => DeviceDetailScreen(product: product),
      ),
    );
  }
  ...
  Widget _buildDeviceGrid() {
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return _buildDeviceCard(product);
      },
    );
  }

  Widget _buildDeviceCard(Product product) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final isEnabled = product.isOnline && product.isOn;

    return GestureDetector(
      onTap: () => _openDeviceDetail(product),
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  // 电源开关
                  GestureDetector(
                    onTap: product.isOnline
                        ? () {
                            AppUtils.vibrate();
                            _toggleDevicePower(_allProducts.indexOf(product));
                          }
                        : null,
                    child: AnimatedContainer(
                      duration: const Duration(milliseconds: 350),
              ...
              AnimatedDefaultTextStyle(
                ...
                child: Text(_getDeviceTypeName(product.type)),
              ),
            ],
          ),
        ),
  ...
  String _getDeviceTypeName(ProductType type) {
    switch (type) {
