# 📚 灵羲 (Lynshae) 文档中心

> 灵羲智能设备 - 完整的项目文档和开发资源

**注意**: 本文档主要针对移动端 App，请先进入 `app/` 目录再进行开发操作。

---

## 🎯 快速导航

### 🚀 开始使用
| 文档 | 描述 | 适合人群 |
|------|------|---------|
| [📖 README](../README.md) | 项目介绍和快速概览 | 所有人 |
| [⚡ 快速开始](../QUICK_START.md) | 5 分钟快速启动项目 | 开发者 |
| [📱 Android 模拟器启动](../QUICK_START.md#-在-android-模拟器中运行详细教程) | 详细的模拟器启动教程 | 新手开发者 |
| [📦 部署指南](../DEPLOYMENT_GUIDE.md) | 从零开始的完整部署教程 | 新手/运维 |

### 📋 项目文档
| 文档 | 描述 | 适合人群 |
|------|------|---------|
| [🎯 项目概述](./PROJECT_OVERVIEW.md) | 项目定位、功能、技术栈 | 产品经理/开发者 |
| [🏗️ 架构设计](./ARCHITECTURE.md) | 分层架构、核心模块 | 架构师/开发者 |
| [✨ 功能特性](./FEATURES.md) | 详细功能说明 | 产品经理/测试 |

### 💻 开发文档
| 文档 | 描述 | 适合人群 |
|------|------|---------|
| [🛠️ 开发指南](./DEVELOPMENT_GUIDE.md) | 开发环境、编码规范 | 开发者 |
| [📊 数据模型](./DATA_MODELS.md) | 数据模型定义与使用 | 开发者 |
| [🔌 蓝牙协议](./BLUETOOTH_PROTOCOL.md) | 蓝牙通信协议规范 | 硬件开发者 |

---

## 🎓 学习路径

### 新手入门

```
1. 阅读 README.md → 了解项目
2. 查看 QUICK_START.md → 快速启动
3. 阅读 PROJECT_OVERVIEW.md → 理解业务
4. 开始编码实践
```

### 开发者深入

```
1. 阅读 ARCHITECTURE.md → 理解架构
2. 查看 DATA_MODELS.md → 理解数据
3. 阅读 DEVELOPMENT_GUIDE.md → 编码规范
4. 研究 BLUETOOTH_PROTOCOL.md → 通信协议
5. 开始功能开发
```

### 运维部署

```
1. 阅读 DEPLOYMENT_GUIDE.md → 环境搭建
2. 查看构建脚本 → 自动化部署
3. 了解监控和日志 → 运维管理
4. 配置 CI/CD → 持续集成
```

---

## 📂 文档结构

```
docs/
├── README.md                    # 文档中心（本文件）
├── PROJECT_OVERVIEW.md          # 项目概述
├── ARCHITECTURE.md              # 架构设计
├── FEATURES.md                  # 功能特性
├── DEVELOPMENT_GUIDE.md         # 开发指南
├── DATA_MODELS.md               # 数据模型
└── BLUETOOTH_PROTOCOL.md        # 蓝牙协议

../
├── README.md                    # 项目主 README
├── QUICK_START.md               # 快速开始
├── DEPLOYMENT_GUIDE.md          # 部署指南
└── scripts/
    ├── start.sh                 # Linux/Mac 启动脚本
    └── start.bat                # Windows 启动脚本
```

---

## 🔍 按主题查找

### 环境搭建
- [DEPLOYMENT_GUIDE.md](../DEPLOYMENT_GUIDE.md) - 完整部署教程
- [QUICK_START.md](../QUICK_START.md) - 快速启动
- [DEVELOPMENT_GUIDE.md](./DEVELOPMENT_GUIDE.md#-开发环境配置) - 开发环境配置

### 架构设计
- [ARCHITECTURE.md](./ARCHITECTURE.md) - 分层架构
- [DATA_MODELS.md](./DATA_MODELS.md) - 数据模型
- [PROJECT_OVERVIEW.md](./PROJECT_OVERVIEW.md#-架构设计) - 架构概览

### 功能开发
- [FEATURES.md](./FEATURES.md) - 功能说明
- [DEVELOPMENT_GUIDE.md](./DEVELOPMENT_GUIDE.md) - 开发规范
- [BLUETOOTH_PROTOCOL.md](./BLUETOOTH_PROTOCOL.md) - 蓝牙协议

### 设备控制
- [BLUETOOTH_PROTOCOL.md](./BLUETOOTH_PROTOCOL.md) - 通信协议
- [DATA_MODELS.md](./DATA_MODELS.md) - 设备模型
- [FEATURES.md](./FEATURES.md#-实时控制) - 控制功能

### 亲密度系统
- [DATA_MODELS.md](./DATA_MODELS.md#bondinglevel-亲密度等级) - 等级定义
- [FEATURES.md](./FEATURES.md#-亲密度养成) - 功能说明
- [PROJECT_OVERVIEW.md](./PROJECT_OVERVIEW.md#-亲密度页-bondingscreen) - 页面说明

### 构建发布
- [DEPLOYMENT_GUIDE.md](../DEPLOYMENT_GUIDE.md#-构建发布版本) - 发布流程
- [DEPLOYMENT_GUIDE.md](../DEPLOYMENT_GUIDE.md#android-发布) - Android 发布
- [DEPLOYMENT_GUIDE.md](../DEPLOYMENT_GUIDE.md#ios-发布) - iOS 发布

---

## 🎯 常见任务

### 我想在 Android 模拟器中运行
→ 查看 [QUICK_START.md](../QUICK_START.md#-在-android-模拟器中运行详细教程)

### 我想快速运行项目
→ 查看 [QUICK_START.md](../QUICK_START.md)

### 我想从零开始配置环境
→ 查看 [DEPLOYMENT_GUIDE.md](../DEPLOYMENT_GUIDE.md)

### 我想了解项目架构
→ 查看 [ARCHITECTURE.md](./ARCHITECTURE.md)

### 我想添加新功能
→ 查看 [DEVELOPMENT_GUIDE.md](./DEVELOPMENT_GUIDE.md)

### 我想修改数据模型
→ 查看 [DATA_MODELS.md](./DATA_MODELS.md)

### 我想了解蓝牙通信
→ 查看 [BLUETOOTH_PROTOCOL.md](./BLUETOOTH_PROTOCOL.md)

### 我想构建发布版本
→ 查看 [DEPLOYMENT_GUIDE.md](../DEPLOYMENT_GUIDE.md#-构建发布版本)

### 我遇到了问题
→ 查看 [DEPLOYMENT_GUIDE.md](../DEPLOYMENT_GUIDE.md#-常见问题排查)

---

## 📊 文档版本

| 文档 | 版本 | 最后更新 |
|------|------|---------|
| README | 1.0.0 | 2026-03-14 |
| QUICK_START | 1.0.0 | 2026-03-14 |
| DEPLOYMENT_GUIDE | 1.0.0 | 2026-03-14 |
| PROJECT_OVERVIEW | 1.0.0 | 2026-03-14 |
| ARCHITECTURE | 1.0.0 | 2026-03-14 |
| FEATURES | 1.0.0 | 2026-03-14 |
| DEVELOPMENT_GUIDE | 1.0.0 | 2026-03-14 |
| DATA_MODELS | 1.0.0 | 2026-03-14 |
| BLUETOOTH_PROTOCOL | 1.0.0 | 2026-03-14 |

---

## 🔄 文档更新记录

### v2.0.0 (2026-03-14)
- ✨ 项目重构：将 APP 内容移至 `app/` 目录
- ✨ 更新项目主 README 说明新结构
- ✨ 更新所有文档路径引用
- ✨ 更新应用名称为"灵羲"

### v1.0.0 (2026-03-04)
- ✨ 创建完整的部署指南 (DEPLOYMENT_GUIDE.md)
- ✨ 创建快速开始指南 (QUICK_START.md)
- ✨ 更新项目主 README
- ✨ 创建文档中心首页
- ✨ 创建自动化启动脚本

---

## 📝 贡献文档

欢迎改进我们的文档！

### 文档规范

1. **清晰简洁** - 使用简单明了的语言
2. **结构完整** - 包含必要的章节和示例
3. **代码示例** - 提供可运行的代码片段
4. **截图说明** - 复杂操作配截图
5. **及时更新** - 保持文档与代码同步

### 提交流程

1. Fork 项目
2. 修改文档
3. 提交更改
4. 创建 Pull Request

---

## 🆘 需要帮助？

如果文档无法解决您的问题：

- 📧 **邮件**: support@lingxi.com
- 🐛 **Issues**: [GitHub Issues](https://github.com/Freakz3z/lynshae_app/issues)

---

## 🔗 外部资源

### 官方文档
- [Flutter 官方文档](https://flutter.dev/docs)
- [Dart 语言指南](https://dart.dev/guides)
- [Android 开发文档](https://developer.android.com/)
- [iOS 开发文档](https://developer.apple.com/)

### 学习资源
- [Flutter 中文网](https://flutter.cn/)
- [Dart 中文网](https://dart.cn/)
- [Flutter GitHub](https://github.com/flutter/flutter)
- [Dart GitHub](https://github.com/dart-lang)

### 社区
- [Stack Overflow - Flutter](https://stackoverflow.com/questions/tagged/flutter)
- [Reddit - r/FlutterDev](https://www.reddit.com/r/FlutterDev/)
- [Flutter 社区](https://flutter.dev/community)

---

<div align="center">
  <strong>📚 文档持续更新中...</strong>
  <br/>
  <sub>最后更新：2026-03-14</sub>
</div>
